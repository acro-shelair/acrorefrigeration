      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {industries.map((ind, i) => (
          <ScrollReveal key={ind.title} delay={i * 80}>
            <Link
              href="/industries"
              className="block bg-card rounded-xl p-5 md:p-6 border border-border hover:border-primary/30 hover:shadow-md transition-all duration-300 group h-full"
            >
              <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center mb-4 group-hover:bg-primary/20 transition-colors duration-300">
                <ind.icon className="w-5 h-5 text-primary" aria-hidden="true" />
              </div>
